 ...
 *#
 *##################################################################################################
 * lcd16x4.c - Linux kernel modules
 *
...
static DEFINE_MUTEX(lcd16x4_mutex);
// Classe do device
...
#define DRV_DESCRIPTION		"LCD 16x4 display driver"
#define DRV_VERSION			"1.5.r02"		// Versão do driver
#define DRV_LICENSE			"GPL v2"		//
#define DRV_NAME			"lcd16x4"		// 
//##################################################################################################
...
/*static int lcd_detect(struct i2c_client *client, struct i2c_board_info *info) {
  ...
  printk(KERN_INFO "[DEBUG][%s] lcd_detect(struct i2c_client *client, struct i2c_board_info *info)\n", DRV_VERSION);
  ...
  if ( (address == 0x21) | (address == 0x23) | (address == 0x25) | (address == 0x27) ) { 
    name = "lcd16x4";
  ...
  } else if ( (address == 0x20) | (address == 0x22) | (address == 0x24) | (address == 0x26) ) { 
    name = "lcd16x4";
  ...
  if (!name) {
    dev_dbg(&adapter->dev, "[DEBUG][%s] Unsupported chip at 0x%02x (man_id=0x%02X, chip_id=0x%02X)\n", DRV_VERSION, address, man_id, chip_id);
    return -ENODEV;
  }
...
static int lcd_probe(struct i2c_client *client, const struct i2c_device_id *id) {
  ...
  if (IS_ERR(lcd_class)) {
    printk(KERN_INFO "[DEBUG][%s] couldn't create sysfs class display\n", DRV_VERSION);
  ...
  if (data->kind == lcd16x4) {
    printk(KERN_INFO "[DEBUG][%s] Create sysfs group file for lcd 16x2.\n", DRV_VERSION);
    ...
    printk(KERN_INFO "[DEBUG][%s] Create sysfs group file for lcd 16x4.\n", DRV_VERSION);
    err = sysfs_create_group(&dev->kobj, &lcd_16x4_group);
    if (err)
      return err;
  ...
  if (IS_ERR(data->hwmon_dev)) {
    printk(KERN_INFO "[DEBUG][%s] Fail to register device.\n", DRV_VERSION);
...
static int lcd_remove(struct i2c_client *client) {
  ...
  printk(KERN_INFO "[DEBUG][%s] lcd_remove()\n", DRV_VERSION);
